<template>
  <div>
    <b-table :items="carisubgroupGetlist" :fields="fields" class="mx-1 mt-2" sort-icon-left >
    ...
    <b-modal id="modal-subcarigroup-edit" ref="modal" title="Cari Grup Güncelleme İşlemi"  @ok="updateItem" no-close-on-backdrop ok-title="Güncelle" cancel-title="Vazgeç">
...
export default {
  ...
  methods: {
    detailOpenModal(data){
      ...
      this.$bvModal.show("modal-subcarigroup-edit")
    },
    updateItem(){
      this.$store.dispatch("cariSubgroupUpdate",this.modalData).then(response => {
        if(response == 200){
    ...
    deleteItem(data){
      this.$bvModal.msgBoxConfirm(data.item.name+" öğesi silinecektir onaylıyor musunuz?", {
        ...
        cancelTitle: 'Vazgeç',
          ...
          .then(value =>{
            if(value){
              this.$store.dispatch("cariSubgroupDelete",data.item)
            }
          })
    }
  },
