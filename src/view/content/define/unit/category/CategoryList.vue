<template>
    <div>
        <b-table :items="categoryGetlist" :fields="fields" class="mx-1 mt-2" sort-icon-left >
        ...
        <b-modal id="modal-category-edit" ref="modal" title="Kategori Güncelleme İşlemi"  @ok="updateCategory" no-close-on-backdrop ok-title="Güncelle" cancel-title="Vazgeç">
    ...
    export default {
        ...
        methods: {
            detailOpenModal(data){
                ...
                this.$bvModal.show("modal-category-edit")
            },
            updateCategory(){
                this.$store.dispatch("categoryUpdate",this.modalData).then(response => {
                    if(response == 200){
            ...
            deleteItem(data){
                this.$bvModal.msgBoxConfirm(data.item.name+" öğesi silinecektir onaylıyor musunuz?", {
                    ...
                    .then(value =>{
                        if(value){
                            this.$store.dispatch("categoryDelete",data.item)
                        }
                    })
            }
        },
