<template>
  <div>
    <b-table :items="carisellerGetlist" :fields="fields" class="mx-1 mt-2" sort-icon-left >
    ...
    <b-modal id="modal-cariseller-edit" ref="modal" title="Kategori Güncelleme İşlemi"  @ok="updateItem" no-close-on-backdrop ok-title="Güncelle" cancel-title="Vazgeç">
...
export default {
  ...
  methods: {
    detailOpenModal(data){
      ...
      this.$bvModal.show("modal-cariseller-edit")
    },
    updateItem(){
      this.$store.dispatch("carisellerUpdate",this.modalData).then(response => {
        if(response == 200){
    ...
    deleteItem(data){
      this.$bvModal.msgBoxConfirm(data.item.name+" öğesi silinecektir onaylıyor musunuz?", {
        ...
        cancelTitle: 'Vazgeç',
          ...
          .then(value =>{
            if(value){
              this.$store.dispatch("carisellerDelete",data.item)
            }
          })
    }
  },
